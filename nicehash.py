...
class private_api:
    ...
    def get_mining_algo_stats(self):
        return self.request('GET', '/main/api/v2/mining/algo/stats/', '', None)

    def send_mining_control_action(self, action):
        if action == 'stop':
            return self.request('POST,' '/main/api/v2/mining/rigs/status2/', '', {'action': "STOP"})
        return self.request('POST,' '/main/api/v2/mining/rigs/status2/', '', {'action': "START"})
